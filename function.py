...
def df_general_stats(df, show_print=False):
    ...
    max_emoji = df.groupby("user").agg({'emoji_count':'sum'}).sort_values(by='emoji_count', ascending=False).head(1)
    ...
    if show_print == True:
        ...
        print("most emoji user:", max_emoji)
    ...
    return n_users, n_messages, n_days, avg_message_per_day, avg_message_per_user, max_len_message, min_len_message, \
        avg_len_message, user_max_len_message, n_words, max_word_message, min_word_message, avg_words, max_emoji
...
def most_emojis(df, n):
    emoji_counts = df['emojis'].apply(pd.Series).stack().value_counts()
    return emoji_counts.head(n)

def most_user_emojis(df, n):

    df_user = df.copy()
    df_user = df[["user", "emoji_count"]]
    df_user = df.groupby("user").sum().reset_index()
    user_emoji_most = df_user.sort_values(by="emoji_count", ascending=False).head(n)
    user_emoji_most.reset_index(inplace=True)
    user_emoji_most.drop(columns="index", inplace=True)
    return user_emoji_most

def most_user_emojis_graphs(df, n):

    top_emoji = most_user_emojis(df,n)

    sns.set_style("darkgrid")

    import matplotlib
    matplotlib.rcParams["figure.figsize"] = (12,8)
    sns.barplot(top_emoji.user, top_emoji.emoji_count, palette="bright")
